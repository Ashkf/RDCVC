...
class CGC(nn.Module):
    def __init__(
        ...
        self.gate_shared = nn.Sequential(
            nn.Linear(
                self.input_size, self.num_specific_experts * 2 + self.num_shared_experts
            ),
            nn.Softmax(dim=1),
        )
        self.gate_task1 = nn.Sequential(
            nn.Linear(
                self.input_size, self.num_specific_experts + self.num_shared_experts
            ),
            nn.Softmax(dim=1),
        )
    ...
    def forward(self, x):
        ...
        gate_task1_out = torch.einsum(
            "abc, ba -> bc", gate_expert_output1, selected_task1
        )
        ...
        gate_task2_out = torch.einsum(
            "abc, ba -> bc", gate_expert_output2, selected_task2
        )

        # gate_shared
        selected_shared = self.gate_shared(inputs_shared)
        gate_expert_outputshared = torch.cat(
            (experts_task1_o, experts_task2_o, experts_shared_o), dim=0
        )
        gate_shared_out = torch.einsum(
            "abc, ba -> bc", gate_expert_outputshared, selected_shared
        )
