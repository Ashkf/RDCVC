"""
* 一种更高效的 KAN 实现。
* github: https://github.com/Blealtan/efficient-kan
*
* File: kan_efficient.py
* Author: Fan Kai
* Soochow University
* Created: 2024-05-29 00:00:19
* ----------------------------
* Modified: 2024-06-08 17:50:40
* Modified By: Fan Kai
* ========================================================================
* HISTORY:
*
* 2024-06-01 11:58:39	FK	一些小的语句调整，无功能性变化。
"""
...
class KANeLinear(torch.nn.Module):
    def __init__(
        self,
        in_features,
        out_features,
        ...
        super(KANeLinear, self).__init__()
...
class KANe(torch.nn.Module):
    def __init__(
        ...
        super(KANe, self).__init__()
        self.grid_size = grid_size
        self.spline_order = spline_order

        self.layers = torch.nn.ModuleList()
        for in_features, out_features in zip(layers_hidden, layers_hidden[1:], strict=False):
            self.layers.append(
                KANeLinear(
                    in_features,
                    out_features,
...
if __name__ == "__main__":
    # Test the KANLinear layer
    layer = KANeLinear(3, 4, grid_size=5, spline_order=3)
    print(layer)
    x = torch.randn(2, 3)
    y = layer(x)
    print(y)
    ...
    print("-" * 20)
    # Test the KAN model
    model = KANe([3, 4, 5], grid_size=5, spline_order=3)
    print(model)
    x = torch.randn(2, 3)
    y = model(x)
    print(y)
