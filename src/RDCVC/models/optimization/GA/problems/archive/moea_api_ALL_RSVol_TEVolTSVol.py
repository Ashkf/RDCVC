...
class MyProblem(ea.Problem):  # 继承 Problem 父类
    def __init__(self, nndir='default', store_data=False, use_existing_data=False):
        ...
        name = 'GAProblem-moea-API[ALL][RSVol][TEVolTSVol]'  # 初始化 name（函数名称，可以随意设置），影响后续结果的保存
    ...
    def aimFunc(self, pop):
        ...
        for i in range(Vars.shape[0]):
            ...
            f_RSVol1 = np.abs(room_supply_volume[0] - ref_room_supply_volume[0])  # 房间 1 送风量偏差
            f_RSVol2 = np.abs(room_supply_volume[1] - ref_room_supply_volume[1])  # 房间 2 送风量偏差
            f_RSVol3 = np.abs(room_supply_volume[2] - ref_room_supply_volume[2])  # 房间 3 送风量偏差
            f_RSVol4 = np.abs(room_supply_volume[3] - ref_room_supply_volume[3])  # 房间 4 送风量偏差
            f_RSVol5 = np.abs(room_supply_volume[4] - ref_room_supply_volume[4])  # 房间 5 送风量偏差
            f_RSVol6 = np.abs(room_supply_volume[5] - ref_room_supply_volume[5])  # 房间 6 送风量偏差

            pop.ObjV[i, :] = np.hstack([f_RSVol1, f_RSVol2, f_RSVol3, f_RSVol4, f_RSVol5, f_RSVol6])
        ...
        c_tsv_max = total_supply_volume - ref_total_supply * 1.1  # 送风量约束，小于设计值 10%
        c_tev = ref_total_exhaust - total_exhaust_volume  # 排风量约束，大于设计值
        c_tev_max = total_exhaust_volume - ref_total_exhaust * 1.1  # 排风量约束，小于设计值 10%

        pop.CV = np.hstack([c_tev, c_tsv, c_tev_max, c_tsv_max])
