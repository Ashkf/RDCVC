...
class MyProblem(ea.Problem):  # 继承 Problem 父类
    def __init__(self, nndir='default', store_data=False, use_existing_data=False):
        ...
        name = 'GAProblem-moea-API[ALL][FreqSVolPres][TEVolTSVol]'  # 初始化 name（函数名称，可以随意设置），影响后续结果的保存
        M = 3  # 初始化 M（目标维数、f 的数量）
    ...
    def aimFunc(self, pop):
        ...
        for i in range(Vars.shape[0]):
            ...
            f_fan = X['AHU_FREQ'] + X['MAU_FREQ'] + X['EF_FREQ']  # 目标函数值：风机频率
            f_room_pres = np.sqrt(MSE(room_pres, room_pres_ref))  # 目标函数值：房间压差偏差
            f_room_supVol = np.sqrt(MSE(room_supply_volume, ref_room_supply_volume))  # 目标函数值：房间送风量偏差
            # f_RSVol1 = np.abs(room_supply_volume[0] - ref_room_supply_volume[0])  # 目标函数值：房间 1 送风量偏差
            # f_RSVol2 = np.abs(room_supply_volume[1] - ref_room_supply_volume[1])  # 目标函数值：房间 2 送风量偏差
            # f_RSVol3 = np.abs(room_supply_volume[2] - ref_room_supply_volume[2])  # 目标函数值：房间 3 送风量偏差
            # f_RSVol4 = np.abs(room_supply_volume[3] - ref_room_supply_volume[3])  # 目标函数值：房间 4 送风量偏差
            # f_RSVol5 = np.abs(room_supply_volume[4] - ref_room_supply_volume[4])  # 目标函数值：房间 5 送风量偏差
            # f_RSVol6 = np.abs(room_supply_volume[5] - ref_room_supply_volume[5])  # 目标函数值：房间 6 送风量偏差

            pop.ObjV[i, :] = np.hstack([f_fan, f_room_pres, f_room_supVol])
        ...
        c_total_supp_vol = ref_total_supply - total_supply_volume  # 送风量约束，大于设计值
        c_total_exh_vol = ref_total_exhaust - total_exhaust_volume  # 排风量约束，大于设计值

        pop.CV = np.hstack([c_total_supp_vol, c_total_exh_vol])
    ...
    def calReferObjV(self):
        referenceObjV = np.array([[0, 0, 0]]).reshape(1, -1)
