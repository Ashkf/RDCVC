...
class SN1(ea.Problem):  # 继承 Problem 父类
    def __init__(self, nndir='default'):
        ...
        name = 'GAProblem-NN[ALL][FreqPres][Pres]'  # 初始化 name（函数名称，可以随意设置），影响后续结果的保存
        M = 1  # 初始化 M（目标维数、f 的数量）
        maxormins = [1]  # 初始化 maxormins（目标最小最大化标记列表，1：min；-1：max）
        Dim = 18  # 初始化 Dim（决策变量维数）
        # =========================== 以下为决策变量的设置 ===========================
        varTypes = [1 for _ in range(Dim)]  # 元素为 0 表示对应的变量是连续的；1 表示是离散的
        lb = [1 for _ in range(Dim)]  # 决策变量下界
        ub = [500, 500, 500] + [90 for _ in range(15)]  # 决策变量上界
        lbin = [0 for _ in range(Dim)]  # 决策变量下边界（0 表示不包含该变量的下边界，1 表示包含）
        ubin = [0 for _ in range(Dim)]  # 决策变量上边界（0 表示不包含该变量的上边界，1 表示包含）
        # =========================== 以上为决策变量的设置 ==========================
        # 调用父类构造方法完成实例化
    ...
    def aimFunc(self, pop):
        ...
        for i in range(Vars.shape[0]):
            ...
            X = DataProcessor.Vars2Dict(Vars[i, :])  # 将决策变量矩阵中的第 i 行转换为一个个体的字典
            ...
            # 风机频率均值
            fan_freq_mean = np.mean([X['MAU_FREQ'], X['AHU_FREQ'], X['EF_FREQ']])
            # 计算目标函数值 (RMSE)，赋值给 pop 种群对象的 ObjV 属性
            pop.ObjV[i, 0] = np.sqrt(MSE(room_pres, eval_ref)) * 0.5 + fan_freq_mean * 0.5
            # 按照房间的压差与设计值比较，计算约束函数值
            c2[i, 0] = np.abs(room_pres[0] - 10) - 8  # RM1_PRES 偏差范围
            c3[i, 0] = np.abs(room_pres[1] - 15) - 8  # RM2_PRES 偏差范围
            c4[i, 0] = np.abs(room_pres[2] - 30) - 8  # RM3_PRES 偏差范围
            c5[i, 0] = np.abs(room_pres[3] - 35) - 8  # RM4_PRES 偏差范围
            c6[i, 0] = np.abs(room_pres[4] - 30) - 8  # RM5_PRES 偏差范围
            c7[i, 0] = np.abs(room_pres[5] - 25) - 8  # RM6_PRES 偏差范围
        ...
        c1 = np.abs(total_SV - ref_total_supply) / ref_total_supply - 0.1  # 送风量约束，偏差不超过 10%
